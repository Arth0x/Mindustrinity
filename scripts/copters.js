...
const t5A_copter = extend(UnitType, "t5A_copter", {
	draw(unit) {
		this.super$draw(unit);
		Draw.rect(
			"mindustrinity-t5A_copter-rotor-1-1",
			unit.x + Angles.trnsx(unit.rotation - 90, 4, 5),
			unit.y + Angles.trnsy(unit.rotation - 90, 4, 5),
			Time.time * 18
		);
		Draw.rect(
			"mindustrinity-t5A_copter-rotor-1-2",
			unit.x + Angles.trnsx(unit.rotation - 90, 4, 5),
			unit.y + Angles.trnsy(unit.rotation - 90, 4, 5),
			Time.time * 18
		);
		Draw.rect(
			"mindustrinity-t5A_copter-rotor-2-1",
			unit.x + Angles.trnsx(unit.rotation - 90, -4, 5),
			unit.y + Angles.trnsy(unit.rotation - 90, -4, 5),
			Time.time * -18
		);
		Draw.rect(
			"mindustrinity-t5A_copter-rotor-2-2",
			unit.x + Angles.trnsx(unit.rotation - 90, -4, 5),
			unit.y + Angles.trnsy(unit.rotation - 90, -4, 5),
			Time.time * -18
		);
